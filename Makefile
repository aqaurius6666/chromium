...
%.zip:
	...
	npm pack
	mkdir -p nodejs/node_modules/@aqaurius6666/chromium/
	tar --directory nodejs/node_modules/@aqaurius6666/chromium/ --extract --file aqaurius6666-chromium-*.tgz --strip-components=1
	npx clean-modules --directory nodejs --include "**/*.d.ts" "**/@types/**" "**/*.@(yaml|yml)" --yes
	rm aqaurius6666-chromium-*.tgz
	mkdir -p $(dir $@)
